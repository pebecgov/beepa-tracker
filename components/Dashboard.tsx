...
import { TOTAL_REFORMS, TOTAL_ACTIVITIES } from "@/convex/beepaFramework";

type ViewMode = "ranking" | "grid" | "cluster";

export function Dashboard() {
  const router = useRouter();
  const { isSignedIn, isAdmin, role, isLoading: userLoading, needsRoleSelection } = useAppUser();
  ...
  if (isSignedIn && needsRoleSelection && !userLoading) {
            ...
            {stats ? (
              <>
                ...
                <StatCard
                  title="Total Reforms"
                  value={TOTAL_REFORMS}
                  subtitle="Reform types in BEEPA framework"
                  icon={
                    <svg className="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {isInitializing ? (
                <>
                  ...
                  <p className="text-gray-500">
                    Creating 60 MDAs with 7 reforms and 52 activities each.
                  </p>
                </>
              ) : (
                <>
